...
export const TopicCard = ({topic,Useraction,history})=>{
    return (
        <div className="topicbox" style={{cursor:"pointer"}}>
            ...
            <img src={`${topic.cover}`} className="topic_image" onClick={()=>history.push(`/topic/${topic.name}`)}>
            </img>
...
const FollowPagecontent = ({tabstate})=>{    
    ...
    return <div className="followtabcontent">{
        tabstate===0&&(
            SuggestedUsers.map((people,i)=><UserCard key={i} people={people} Useraction={Useraction} history={history} isMe={data?.username===people.username}/>)
        )        
    }
    ...
    {
        tabstate===1&&(
            suggestedtopics.map((topic,i)=><TopicCard key={i} topic={topic} Useraction={Useraction} history={history}/>)
        )
    }
    </div>
    </div>
